...
export default async function handler(req, res) {
  ...
  if (!clientId || !clientSecret) {
    return res.status(500).json({ error: 'Server misconfigured: BLING_CLIENT_ID or BLING_CLIENT_SECRET missing in env' });
  }

  let body = {};
  try {
    body = req.body && Object.keys(req.body).length ? req.body : req.query || {};
  } catch {
    body = req.query || {};
  }
  ...
  if (grantType === 'authorization_code' && !code) {
    return res.status(400).json({ error: 'authorization_code grant requires code' });
  }
  if (grantType === 'refresh_token' && !refreshToken) {
    return res.status(400).json({ error: 'refresh_token grant requires refresh_token' });
  }

  const tokenUrl = 'https://api.bling.com.br/Api/v3/oauth/token';

  const NodeBuffer = globalThis['Buffer'];
  const auth = NodeBuffer ? NodeBuffer.from(`${clientId}:${clientSecret}`).toString('base64') : btoa(`${clientId}:${clientSecret}`);

  const params = new URLSearchParams();
  params.append('grant_type', grantType);
  if (grantType === 'authorization_code') params.append('code', code);
  if (grantType === 'refresh_token') params.append('refresh_token', refreshToken);
  ...
  try { data = JSON.parse(text); } catch { data = { raw: text }; }
    ...
    if (!resp.ok) {
      return res.status(502).json({ error: 'Failed to exchange token', status: resp.status, data });
    }
