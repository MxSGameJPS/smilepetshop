...
export default async function handler(req, res) {
  try {
    ...
    if (params && params.code && procEnv.BLING_CLIENT_ID && procEnv.BLING_CLIENT_SECRET) {
      try {
        const NodeBuffer = globalThis['Buffer'];
        const auth = NodeBuffer ? NodeBuffer.from(procEnv.BLING_CLIENT_ID + ':' + procEnv.BLING_CLIENT_SECRET).toString('base64') : btoa(procEnv.BLING_CLIENT_ID + ':' + procEnv.BLING_CLIENT_SECRET);
        ...
        const tokenResp = await fetch('https://api.bling.com.br/Api/v3/oauth/token', {
          ...
          body: new URLSearchParams({ grant_type: 'authorization_code', code: params.code }).toString(),
        });

        const txt = await tokenResp.text();
        try { tokenData = JSON.parse(txt); } catch { tokenData = { raw: txt } }
      } catch {
        tokenData = null;
      }
    }
